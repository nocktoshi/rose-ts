...
const fromB58 = (s: string): Uint8Array => base58.decode(s);

const wasmKeyPlain = (key: {
...
const tsKeyPlain = (key: RoseTs.ExtendedKey) => ({
  privateKey: key.privateKey ? new Uint8Array(key.privateKey) : undefined,
  publicKey: new Uint8Array(key.publicKey),
  chainCode: new Uint8Array(key.chainCode),
});
...
describe('parity: slip10', () => {
  it('deriveMasterKey matches wasm on mnemonic seed', async () => {
    ...
    expectParity(
      ...
      wasmKeyPlain(wasmKey),
      tsKeyPlain(tsKey),
    );
    expectBytesEqual(
      must(tsKey.privateKey, 'master private key'),
      fromB58('3MoHxVXWAr9qny12Sw8ZZtrgEBFcZegQQVkwYyePb9LZ'),
    );
    expectBytesEqual(
      tsKey.chainCode,
      fromB58('3NhBRdy7vRw8vKQ5RnR3CNcD43WDn5Ky7mhhotqUcaiR'),
    );
  });
  ...
  it('deriveChild matches wasm (normal + hardened)', async () => {
    ...
    expectParity(
      'deriveChild(0)',
      wasmKeyPlain(wasmChild),
      tsKeyPlain(tsChild),
    );
    expectBytesEqual(
      must(tsChild.privateKey, 'child private key'),
      fromB58('6AifHLAuT1MxnFsoCwjKNFaBze91DXFDV1rRLefkzPEK'),
    );
    ...
    expectParity(
      ...
      wasmKeyPlain(wasmHardened),
      tsKeyPlain(tsHardened),
    );
    expectBytesEqual(
      must(tsHardened.privateKey, 'hardened private key'),
      fromB58('CpMAmcgN1V6Majtx2HC7ULLXD9psA3Gg3nMye3JpKpH'),
    );
  });
  ...
  it('signMessage digest matches nockchain-wallet vector', async () => {
    ...
    const tsSig = RoseTs.signMessage(
      must(tsKey.privateKey, 'signing private key'),
      message,
    );
