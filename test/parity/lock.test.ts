...
function htlcOrLockWasm(wasm: Awaited<ReturnType<typeof getWasm>>) {
...
describe("parity: lock", () => {
  it("pkhSingle matches wasm", async () => {
    ...
    expectParity("pkhSingle", wasm.pkhSingle(BUYER_PKH), RoseTs.pkhSingle(BUYER_PKH));
  });
  ...
  it("spendConditionFirstName matches wasm", async () => {
    ...
    expectParity(
      ...
      RoseTs.spendConditionFirstName(RoseTs.spendConditionNewPkh(RoseTs.pkhSingle(BUYER_PKH)))
    );
  });
  ...
  it("HTLC lockRootHash matches wasm", async () => {
    ...
    expectParity("lockRootHash", wasm.lockRootHash(wasmLock), RoseTs.lockRootHash(tsLock));
  });
