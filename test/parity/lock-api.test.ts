...
describe("parity: lock APIs", () => {
  it("lockHeight matches wasm", async () => {
    ...
    expectParity("lockHeight", wasm.lockHeight(lock), RoseTs.lockHeight(lock as never));
  });

  it("spendConditionHash matches wasm", async () => {
    const wasm = await getWasm();
    const sc = wasm.spendConditionNewPkh(wasm.pkhSingle(BUYER_PKH));
    expectParity("spendConditionHash", wasm.spendConditionHash(sc), RoseTs.spendConditionHash(sc as never));
  });
  ...
  it("lockProve matches wasm on single-PKH lock", async () => {
    ...
    const sc = wasm.spendConditionNewPkh(wasm.pkhSingle(BUYER_PKH));
    const lock = wasm.lockFromList([sc]);
    expectParity("lockProve", wasm.lockProve(lock, 0), RoseTs.lockProve(lock as never, 0));
  });
