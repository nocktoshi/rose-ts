...
const htlcNotePb = (assets: string) => ({
  note_version: {
    V1: {
      version: {value: '1'},
...
describe('parity: simpleSpend HTLC / multisig variants', () => {
  it('simpleSpendHtlc refund (index 1) builds balanced tx', async () => {
    ...
    if (sb.spend.tag === 1) {
      expect(
        sb.spend.witness.lock_merkle_proof.spend_condition.some(
          p => p.tag === 'tim',
        ),
      ).toBe(true);
    }
  ...
  it('low-level HTLC claim spend recalcAndSetFee after addPreimage', async () => {
    ...
    expectParity(
      'rawTxV1CalcId HTLC claim with hax',
      wasm.rawTxV1CalcId(wasmRaw),
      RoseTs.rawTxV1CalcId(tsRaw),
    );
  });
  ...
  it('simpleSpendHtlc claim attaches structural hax preimage', async () => {
    ...
    expectParity(
      'simpleSpendHtlc vs withLocks claim',
      tx.id,
      withLocks.build().id,
    );
    void wasm;
  ...
  it('simpleSpendHtlc attaches memo/blob on output seed', async () => {
    ...
    const recipientSeed = must(
      spend.seeds.find(s => s.gift === gift),
      'recipient seed',
    );
    expect(recipientSeed.note_data.map(([k]) => k)).toEqual([
      'lock',
      'blob',
      'memo',
    ]);
    expect(
      RoseTs.decodeNoteDataPackedUtf8(
        mustAt(recipientSeed.note_data, 1)[1] as never,
      ),
    ).toBe('payload');
    expect(
      RoseTs.decodeNoteDataPackedUtf8(
        mustAt(recipientSeed.note_data, 2)[1] as never,
      ),
    ).toBe('htlc-claim');
    void wasm;
  });
  ...
  it('multisigLock matches wasm m-of-n shape', async () => {
    ...
    const wasmPkh = (
      wasmLock as {tag: string; m: number; hashes: unknown[]}[]
    ).find(p => p.tag === 'pkh');
    const tsPkh = (
      tsLock as {tag: string; m: number; hashes: unknown[]}[]
    ).find(p => p.tag === 'pkh');
    expect(wasmPkh?.m).toBe(2);
    expect(tsPkh?.m).toBe(2);
    expect(tsPkh?.hashes).toHaveLength(3);
    expectParity(
      'multisigLock wasm root',
      wasm.lockRootHash(wasmLock),
      wasm.lockRootHash(tsLock),
    );
  });
