...
describe("COMPAT: lock.ts build matches wasm (loose seed, no output_source)", () => {
  it("rose-ts TxBuilder.build() is structurally identical to wasm", async () => {
    ...
    const buildTs = () => {
      ...
      spend.seed({ lock_root: lockRoot, note_data: [], gift: giftNicks, parent_hash: RoseTs.noteHash(note) } as never);
    ...
    const buildWasm = () => {
      ...
      spend.seed({ lock_root: lockRoot, note_data: [], gift: giftNicks, parent_hash: wasm.noteHash(note) } as never);
      spend.computeRefund(false);
  ...
  it("rose-ts noteFromProtobuf matches wasm (the notes[] passed to nock_signTx)", async () => {
    ...
    const norm = (v: unknown) =>
      JSON.parse(JSON.stringify(v, (_, x) => (typeof x === "bigint" ? x.toString() : x)));
    expect(norm(RoseTs.noteFromProtobuf(pb))).toEqual(norm(wasm.noteFromProtobuf(pb as never)));
  });
});
