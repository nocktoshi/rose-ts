...
describe("note-data memo/blob (PR #116 packed belts)", () => {
  it("noteDataPushBlob round-trips UTF-8 and matches explorer jam decode", () => {
    ...
    const explorerJam = Uint8Array.from(atob(EXPLORER_BLOB_B64), (c) => c.charCodeAt(0));
    expect(RoseTs.decodePackedBlobUtf8(explorerJam)).toBe(BLOB_UTF8);
    expect(RoseTs.decodePackedBlobUtf8(jam(mustAt(data, 0)[1]))).toBe(BLOB_UTF8);
  });
  ...
  it("noteDataPushMemo encodes UTF-8 and preserves lock ordering", () => {
    ...
    let data = RoseTs.noteDataPushPkh(RoseTs.noteDataEmpty(), RoseTs.pkhSingle(BUYER_PKH));
    ...
    expect(RoseTs.decodeNoteDataPackedUtf8(mustAt(data, 2)[1])).toBe("hello memo");
  });

  it("rejects empty memo", () => {
    expect(() => RoseTs.noteDataPushMemo(RoseTs.noteDataEmpty(), "")).toThrow(/empty/i);
  });
});