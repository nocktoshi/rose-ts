...
export const expectBytesEqual = (a: Uint8Array, b: Uint8Array): void => {
 ...
 */
export const expectParity = <T>(label: string, wasmOut: T, tsOut: T): void => {
  try {
    expectDeepEqual(tsOut, wasmOut);
  } catch (err) {
    throw new Error(`parity mismatch (${label}): ${(err as Error).message}`, {
      cause: err,
    });
  }
};
