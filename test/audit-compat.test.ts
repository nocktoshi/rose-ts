...
function inputNotePb(assets: string) {
  return {
    note_version: {
      V1: {
        version: { value: "1" },
...
describe("COMPAT: pinned golden vectors (atomic-nock rose.test.ts.snap)", () => {
  it("hashPreimage / hashPublicKey / htlcLockRootDigest match pinned values", () => {
    ...
    expect(hNock).toBe("8XiEzPMGNQp29EwSdtGhHsyEmXsDR2AkZfuTWCfydWVA8XbKsLk7BGo");
    expect(buyerPkh).toBe("ey4Lwommv6EeDfZzMrNKf7pJzShfoiCxJh7hEcoKu9TfzaXxngcwHJ");
    expect(sellerPkh).toBe("gFz59ms5byUAp4kbgatYHZFve3ZxMSqspGPUVweyP1u4XQCzLjsdKp");
  ...
  it("spendConditionFirstName (wallet note lookup) matches wasm", async () => {
    ...
    expect(RoseTs.spendConditionFirstName(sc)).toBe(wasm.spendConditionFirstName(sc));
  });
});
...
describe("COMPAT: note protobuf roundtrip + field shapes (claim/refund/balance)", () => {
  it("noteFromProtobuf(noteToProtobuf(note)) is stable and matches wasm; .assets/.name usable", async () => {
    ...
    const rt = RoseTs.noteFromProtobuf(RoseTs.noteToProtobuf(note as never) as never);
    expect(RoseTs.noteHash(rt)).toBe(RoseTs.noteHash(note as never));
    expect(RoseTs.noteHash(note as never)).toBe(wasm.noteHash(wasm.noteFromProtobuf(pb as never)));
  });
});
