...
describe("AUDIT: encodeLock includes version tag", () => {
  ...
  function scList(n: number) {
    return KEYS.slice(0, n).map((k) => RoseTs.spendConditionNewPkh(RoseTs.pkhSingle(k)));
  }
...
describe("AUDIT: spends ZMap ordering (multi-input tx id)", () => {
  function namedNotePb(first: string, last: string, assets: string) {
    return {
      note_version: {
        V1: {
          version: { value: "1" },
  ...
  it("two-input simpleSpend tx id matches wasm", async () => {
    ...
    const build = (engine: typeof RoseTs | typeof wasm) => {
      ...
      const locks = notes.map(() => ({
        lock: engine.lockFromList([engine.spendConditionNewPkh(engine.pkhSingle(spender))]),
        lock_sp_index: 0,
      }));
      const b = new engine.TxBuilder(settings as never);
      b.simpleSpend(notes as never, locks as never, recipient, "2500000" as never, null, refund, false);
      return b.build();
    };
  ...
  for (const includeLockData of [false, true]) {
    it(`three equal-gift seeds (includeLockData=${includeLockData}) hash like wasm`, async () => {
      ...
      const tsSeeds = seedsArgs.map(([pkh, gift]) =>
        RoseTs.seedV1NewSinglePkh(pkh, gift as RoseTs.Nicks, parent, includeLockData)
      );
      ...
      expect(RoseTs.hashSeedsV1Digest(tsSeeds)).toBe(wasm.seedsV1Hash(wasmSeeds));
    });
  }
});
