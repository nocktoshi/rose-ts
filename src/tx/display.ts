...
const findInZMap = <T>(inputs: [Name, T][], name: Name): T | undefined => {
...
export const getDisplayInput = (
  ...
  if ('inputs' in display && !('tag' in display)) {
    const entry = findInZMap(
      display.inputs as [
        Name,
        SpendCondition | {tag: 'v0'; sig: unknown} | unknown,
      ][],
      name,
    );
...
export const isV1DisplayInput = (
  input: ReturnType<typeof getDisplayInput>,
): input is SpendCondition =>
  input !== undefined &&
  !(typeof input === 'object' && 'tag' in input && input.tag === 'v0');
