...
const spendConditionPkh = (sc: SpendCondition) =>
  sc.filter(p => p.tag === 'pkh');

const spendConditionHax = (sc: SpendCondition) =>
  sc.filter(p => p.tag === 'hax');

const spendConditionBrn = (sc: SpendCondition): boolean =>
  sc.some(p => p.tag === 'brn');

/** rose-nockchain-types `SpendBuilder::missing_unlocks`. */
export const computeMissingUnlocks = (spend: SpendV1): MissingUnlocks[] => {
  if (spend.tag !== 1) return [];
  const s = spend as Spend1V1;
  const missing: MissingUnlocks[] = [];
  const presentSigs = new Set(
    (Array.isArray(s.witness.pkh_signature) ? s.witness.pkh_signature : []).map(
      ([pkh]) => pkh,
    ),
  );
  ...
  for (const h of spendConditionHax(sc)) {
    const valid = new Set(
      Array.isArray(h.preimages) ? (h.preimages as Digest[]) : [],
    );
    const current = new Set(
      (Array.isArray(s.witness.hax_map) ? s.witness.hax_map : []).map(
        ([digest]) => digest,
      ),
    );
