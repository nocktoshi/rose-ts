...
export const spendV1NewWitness = (
  witness: Witness,
  seeds: SeedsV1,
  fee: Nicks,
): SpendV1 => {
...
export const spendV1NewLegacy = (seeds: SeedsV1, fee: Nicks): SpendV1 => ({
  tag: 0,
...
export const spendV1SigHash = (spend: SpendV1): Digest => {
  if (spend.tag === 1) {
    return hashSpendV1SigHash(spend);
  }
  return hashToDigest(
    hashTuple(
      digestFromBase58(seedsV1SigHash(spend.seeds)),
      hashNicks(spend.fee),
    ),
  );
};
