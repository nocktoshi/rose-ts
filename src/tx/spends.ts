...
export const nameKey = (name: Name): string =>
  `${name.first}:${name.last}:${name._sig ?? 0}`;

const cloneWitness = (w: Witness): Witness => ({
...
const stripWitness = (w: Witness): Witness => ({
  lock_merkle_proof: w.lock_merkle_proof,
  pkh_signature: [],
  hax_map: [],
  tim: null,
...
export const splitWitness = (
  spends: SpendsV1,
): {spends: SpendsV1; witnessData: WitnessData} => {
