import type { Digest, Lock, Nicks, Note, NockchainTx, RawTxV1, TxEngineSettings } from "../types.js";
...
function nockchainTxToRawTx(tx: NockchainTx): RawTxV1 {
...
export function htlcLockRootDigest(
  hNock: Digest,
  buyerPkh: Digest,
  sellerPkh: Digest,
  refundHeight: bigint
...
export function htlcGiftOutputFirstName(params: {
  ...
  const inputLock = lockFromList([
    spendConditionNewPkh(pkhSingle((params.inputPkh ?? params.sellerPkh) as Digest)),
  ]);
