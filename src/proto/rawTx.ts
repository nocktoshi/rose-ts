...
import {nounIsBased} from '../hash/structural.js';
...
const digestField = (value: unknown, field: string): Digest => {
...
const nicksField = (value: unknown, field: string): string => {
...
const beltsToLeBytes = (belts: readonly bigint[]): Uint8Array => {
...
const sourceFromPb = (pb: {hash: string; is_coinbase?: boolean}): Source => ({
  hash: pb.hash as Digest,
  is_coinbase: pb.is_coinbase ?? false,
});

const noteDataFromPb = (
  pb: {entries?: {key: string; blob: number[]}[]} | null | undefined,
): NoteData => {
  ...
  return entries.map(e => {
    ...
    if (!nounIsBased(tree))
      throw new Error('NoteData value is not based (atom outside the field)');
    return [e.key, toWire(tree) as Noun];
  });
};

const lockMerkleProofFromPb = (pb: {
  ...
  const spendCondition = spendConditionFromProtobuf(
    required(
      pb.spend_condition,
      'LockMerkleProof.spend_condition',
    ) as Parameters<typeof spendConditionFromProtobuf>[0],
  );
...
const legacySignatureFromPb = (pb: {
  entries?: {
  ...
  return entries.map(entry => {
    const pubkeyPb = required(
      entry.schnorr_pubkey?.value,
      'LegacySignatureEntry.schnorr_pubkey',
    );
    const pubkey = cheetahPointToBase58(
      cheetahPointFromPb(pubkeyPb as Parameters<typeof cheetahPointFromPb>[0]),
    );
    const signature = signatureFromPb(
      required(entry.signature, 'LegacySignatureEntry.signature'),
    );
    return [pubkey, signature];
  });
};

const pkhSignatureFromPb = (pb: {
  entries?: {
    hash?: unknown;
  ...
  return entries.map(entry => {
    ...
    const signature = signatureFromPb(
      required(entry.signature, 'PkhSignatureEntry.signature'),
    );
    return [hash, [pubkey, signature]] as PkhSignature[number];
  });
};

const witnessFromPb = (pb: {
  lock_merkle_proof?: unknown;
  pkh_signature?: unknown;
  hax?: {hash?: unknown; value: number[]}[];
}): Witness => {
  const lockMerkleProof = lockMerkleProofFromPb(
    required(pb.lock_merkle_proof, 'Witness.lock_merkle_proof') as Parameters<
      typeof lockMerkleProofFromPb
    >[0],
  );
  const pkhSignature = pkhSignatureFromPb(
    required(pb.pkh_signature, 'Witness.pkh_signature') as Parameters<
      typeof pkhSignatureFromPb
    >[0],
  );
  ...
  for (const hax of pb.hax ?? []) {
    ...
    if (!nounIsBased(tree))
      throw new Error(
        'HaxPreimage value is not based (atom outside the field)',
      );
...
const nameFromPb = (pb: {first: string; last: string}): Name => ({
  first: pb.first as Digest,
  last: pb.last as Digest,
  _sig: 0,
});

const spendFromPb = (pb: {spend_kind?: Record<string, unknown>}): SpendV1 => {
  ...
  if ('Witness' in kind) {
    ...
    const witness = witnessFromPb(
      required(w.witness, 'WitnessSpend.witness') as Parameters<
        typeof witnessFromPb
      >[0],
    );
    const seeds = (w.seeds ?? []).map(s =>
      seedFromPb(s as Parameters<typeof seedFromPb>[0]),
    );
    const fee = nicksField(
      required(w.fee, 'WitnessSpend.fee'),
      'WitnessSpend.fee',
    );
  ...
  if ('Legacy' in kind) {
    ...
    const signature = legacySignatureFromPb(
      required(l.signature, 'LegacySpend.signature') as Parameters<
        typeof legacySignatureFromPb
      >[0],
    );
    ...
    const fee = nicksField(
      required(l.fee, 'LegacySpend.fee'),
      'LegacySpend.fee',
    );
...
export const rawTxFromProtobuf = (tx: PbCom2RawTransaction): RawTxV1 => {
  ...
  const spends: SpendsV1 = (tx.spends ?? []).map(entry => {
    const name = nameFromPb(
      required(entry.name, 'SpendEntry.name') as {first: string; last: string},
    );
    const spend = spendFromPb(
      required(entry.spend, 'SpendEntry.spend') as {
        spend_kind?: Record<string, unknown>;
      },
    );
