...
const requiredBelt = (h: PbCom1Hash, field: keyof PbCom1Hash): bigint => {
  const belt = h[field];
  if (!belt?.value) {
...
export const digestFromProtobuf = (h: PbCom1Hash): Digest => {
  const belts = [
...
export const digestToProtobuf = (d: Digest): PbCom1Hash => {
