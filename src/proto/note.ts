...
const parseVersion = (value: string | undefined): Version => {
...
const noteDataFromPb = (
  ...
  return entries.map(e => {
    const noun = cue(new Uint8Array(e.blob));
    if (!nounIsBased(fromWire(noun))) {
...
const noteDataToPb = (
  data: NoteData,
): {entries: {key: string; blob: number[]}[]} => {
...
const isNoteV1 = (note: Note): note is NoteV1 =>
  'origin_page' in note && !('inner' in note);

export const noteFromProtobuf = (value: PbCom2Note): Note => {
  ...
  if ('V1' in version) {
    ...
    return {
      ...
      origin_page: Number(
        required(v1.origin_page?.value, 'origin_page'),
      ) as BlockHeight,
      name: parseName(
        required(v1.name, 'name') as {first: string; last: string},
      ),
      note_data: noteDataFromPb(
        v1.note_data as {entries?: {key: string; blob: number[]}[]},
      ),
  ...
  if ('Legacy' in version) {
    ...
    return {
      inner: {
        ...
        origin_page: Number(
          required(legacy.origin_page?.value, 'origin_page'),
        ) as BlockHeight,
        timelock: {tim: null},
      },
      name: parseName(
        required(legacy.name, 'name') as {first: string; last: string},
      ),
...
export const noteToProtobuf = (note: Note): PbCom2Note => {
  if (isNoteV1(note)) {
    return {
      note_version: {
        V1: {
