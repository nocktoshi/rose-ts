...
const f6Eq = (a: F6lt, b: F6lt): boolean => a.every((v, i) => v === b[i]);

const karat3 = (a: Belt[], b: Belt[]): Belt[] => {
  const m = [
  ...
  return [
    ...
    badd(
      bsub(
        bmul(
          badd(mustAt(a, 0), mustAt(a, 2)),
          badd(mustAt(b, 0), mustAt(b, 2)),
        ),
...
export const f6Mul = (f: F6lt, g: F6lt): F6lt => {
  const f0g0 = karat3(
  ...
  const foil = karat3(
    [
      badd(mustAt(f, 0), mustAt(f, 3)),
      badd(mustAt(f, 1), mustAt(f, 4)),
      badd(mustAt(f, 2), mustAt(f, 5)),
    ],
    [
      badd(mustAt(g, 0), mustAt(g, 3)),
      badd(mustAt(g, 1), mustAt(g, 4)),
      badd(mustAt(g, 2), mustAt(g, 5)),
    ],
  );
  const cross = [
...
const bpscal = (scalar: Belt, b: Belt[], res: Belt[]): void => {
  for (let i = 0; i < b.length; i++) res[i] = bmul(scalar, mustAt(b, i));
};

export const f6Inv = (f: F6lt): F6lt | null => {
...
const f6Div = (f: F6lt, g: F6lt): F6lt | null => {
  const gInv = f6Inv(g);
  if (!gInv) return null;
  return f6Mul(f, gInv);
};

const f6Add = (f1: F6lt, f2: F6lt): F6lt =>
  f1.map((v, i) => badd(v, mustAt(f2, i))) as F6lt;

const f6Neg = (f: F6lt): F6lt => f.map(v => bneg(v)) as F6lt;

const f6Sub = (f1: F6lt, f2: F6lt): F6lt =>
  f1.map((v, i) => bsub(v, mustAt(f2, i))) as F6lt;

const f6Scal = (s: Belt, f: F6lt): F6lt => f.map(v => bmul(s, v)) as F6lt;

const f6Square = (f: F6lt): F6lt => f6Mul(f, f);

const pointsEqual = (a: CheetahPoint, b: CheetahPoint): boolean =>
  a.inf === b.inf && f6Eq(a.x, b.x) && f6Eq(a.y, b.y);

const chDoubleUnsafe = (x: F6lt, y: F6lt): CheetahPoint | null => {
...
const chDouble = (p: CheetahPoint): CheetahPoint | null => {
  if (p.inf || f6Eq(p.y, F6_ZERO))
    return {...A_ID, x: [...A_ID.x], y: [...A_ID.y]};
  return chDoubleUnsafe(p.x, p.y);
};

const chAddUnsafe = (p: CheetahPoint, q: CheetahPoint): CheetahPoint | null => {
...
export const chNeg = (p: CheetahPoint): CheetahPoint => ({
  x: [...p.x],
  y: f6Neg(p.y),
  inf: p.inf,
});
...
export const chScalBig = (n: U256, p: CheetahPoint): CheetahPoint | null => {
...
export const cheetahPointToBase58 = (point: CheetahPoint): string =>
  base58.encode(publicKeyToBeBytes(point));

export const cheetahPointFromBase58 = (b58: string): CheetahPoint => {
...
export const publicKeyToBeBytes = (point: CheetahPoint): Uint8Array => {
...
export const publicKeyToSlip10Bytes = (point: CheetahPoint): Uint8Array => {
  const out = new Uint8Array(96);
  let offset = 0;
  for (const belt of [
    ...point.y.slice().reverse(),
    ...point.x.slice().reverse(),
  ]) {
    for (let j = 0; j < 8; j++) {
...
export const publicKeyFromBeBytes = (bytes: Uint8Array): CheetahPoint => {
...
export const cheetahPointHash = (point: CheetahPoint): string => {
  const dyck = [
    0n,
    0n,
    1n,
