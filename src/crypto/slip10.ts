...
const scalarFromPrivateBytes = (bytes: Uint8Array): U256 =>
  U256.fromBeBytes(bytes);

const publicKeyFromScalar = (scalar: U256): CheetahPoint => {
  const pt = chScalBig(scalar, A_GEN);
  if (!pt) throw new Error('invalid private key scalar');
  return pt;
};

const indexBytes = (index: number): Uint8Array => {
...
export const deriveMasterKey = (seed: Uint8Array): ExtendedKey => {
...
export const deriveMasterKeyFromMnemonic = (
  mnemonic: string,
