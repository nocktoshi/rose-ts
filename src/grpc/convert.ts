...
import type { Digest, PbCom2Balance, PbCom2BalanceEntry, PbCom2Note, PbCom2NoteV1 } from "../types.js";
import type { Balance as GrpcBalance, Note as GrpcNote } from "./gen/nockchain/common/v2/blockchain.js";
...
function grpcHashToPb(h: Hash): PbCom1Hash {
  const pb: PbCom1Hash = {};
...
function grpcHashToDigest(h: Hash | undefined): string {
  if (!h) return "";
  return digestFromProtobuf(grpcHashToPb(h));
}

function grpcNameToPb(name: GrpcName | undefined): { first: string; last: string } | undefined {
  if (!name?.first || !name.last) return undefined;
  return { first: grpcHashToDigest(name.first), last: grpcHashToDigest(name.last) };
}

function grpcNoteToPb(note: GrpcNote | undefined): PbCom2Note | null {
...
function balanceEntryToPb(entry: GrpcBalance["notes"][number]): PbCom2BalanceEntry {
...
export function grpcBalanceToPb(balance: GrpcBalance): PbCom2Balance {
...
export function digestToGrpcHash(digest: string): Hash {
...
export function nameToGrpcName(name: { first: string; last: string }): GrpcName {
  return {
