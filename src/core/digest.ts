...
export const beltsToUint = (belts: DigestBelts): bigint => {
  let result = 0n;
  let power = 1n;
...
export const digestToBase58 = (belts: DigestBelts): string => {
...
export const digestFromBelts = (belts: bigint[]): DigestBelts =>
  [
    mustAt(belts, 0),
    mustAt(belts, 1),
    mustAt(belts, 2),
...
export const digestFromBase58 = (s: string): DigestBelts => {
...
export const digestBeltsToBase58 = (belts: DigestBelts): string =>
  digestToBase58(belts);

/** 40-byte big-endian representation (rose-ztd `Digest::to_bytes`). */
export const digestBeltsToBytes = (belts: DigestBelts): Uint8Array => {
  let hex = beltsToUint(belts).toString(16);
  if (hex.length % 2 !== 0) hex = '0' + hex;
