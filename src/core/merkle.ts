...
import { hashPair, hashSpendCondition, hashToDigest, hashU64 } from "./hashable.js";
...
const pair = (left: HashTree, right: HashTree): HashTree => ({ kind: "pair", left, right });

/** Build the inner spend-condition subtree of a balanced `LockVN` node. */
function innerTree(node: unknown, depth: number): HashTree {
...
function lockTree(lock: Lock): HashTree {
...
function treeHash(t: HashTree): DigestBelts {
  if (t.kind === "leaf") return t.hash;
  return hashPair(treeHash(t.left), treeHash(t.right));
}

function treeLeafCount(t: HashTree): number {
...
function proveTree(t: HashTree, index: number): MerkleProvenAxis {
  if (t.kind === "leaf") {
...
export function proveHashableLock(lock: Lock, index: number): MerkleProvenAxis {
  return proveTree(lockTree(lock), index);
}
