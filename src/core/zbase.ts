...
const tipEq = <K>(a: K, b: K, keyNoun: (k: K) => NounTree): boolean =>
  nounOrderDigest(keyNoun(a)) === nounOrderDigest(keyNoun(b));

const gorTip = <K>(a: K, b: K, keyNoun: (k: K) => NounTree): boolean => {
...
const doubleTip = <K>(key: K, keyNoun: (k: K) => NounTree): Uint8Array => {
...
const morTip = <K>(a: K, b: K, keyNoun: (k: K) => NounTree): boolean =>
  compareBytes(doubleTip(a, keyNoun), doubleTip(b, keyNoun)) < 0;
...
export const hashZSetSingleton = <E>(
  entry: E,
 ...
 */
export const hashZSetDigests = (digests: readonly string[]): DigestBelts => {
