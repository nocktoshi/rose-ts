...
const hashNounWire = (noun: NounWire): DigestBelts =>
  digestFromBase58(hashNounStructural(fromWire(noun)));

export const hashNoteData = (data: NoteData): DigestBelts => {
...
export const noteDataFeeWords = (data: NoteData): bigint => {
  let w = 1n;
...
export const noteDataPushPkh = (data: NoteData, pkh: Pkh): NoteData => {
  const filtered = data.filter(([k]) => k !== 'lock');
  const noun = toWire(
    encodeTuple([
      encodeAtomU64(0n),
      cons(tasBelts('pkh'), encodePkh(pkh)),
      encodeAtomU64(0n),
    ]),
  );
...
export const noteDataPushLock = (
  ...
  return [
    ...filtered,
    ['lock', toWire(encodeTuple([encodeAtomU64(0n), fromWire(lockNoun)]))],
  ];
};
...
const hashDigestField = (d: Digest): DigestBelts => digestFromBase58(d);

const isHashSource = (
  source: Source,
): source is {hash: Digest; is_coinbase: boolean} => 'hash' in source;

export const hashSourceFields = (source: Source): Digest => {
  if (!isHashSource(source)) {
    throw new Error('Parent source variant is not hashable as legacy Source');
  }
  return hashToDigest(
    hashTuple(hashDigestField(source.hash), hashBool(source.is_coinbase)),
  );
};

export const nameHash = (name: Name): Digest =>
  hashToDigest(hashNameBelts(name));

export const nameV1 = (lock: Digest, source: Source): Name => {
  ...
  const last = hashToDigest(
    hashTuple(
      hashBool(true),
      hashTuple(hashDigestField(sourceHashed), hashU64(0n)),
    ),
  );
  return {first, last, _sig: 0};
};

const hashNoteV1 = (note: NoteV1): Digest =>
  hashToDigest(
    hashTuple(
...
const hashNoteV0 = (note: NoteV0): Digest => {
  ...
  void sig;
  const sourceHash = hashTuple(
    hashDigestField(note.source.hash),
    hashBool(note.source.is_coinbase),
  );
  return hashToDigest(
    hashTuple(
      innerHash,
...
export const noteHash = (note: Note): Digest => {
