...
export function encodeBlobBelts(bytes: Uint8Array): bigint[] {
...
function beltSeqFromPackedNoun(noun: NounTree): bigint[] {
  const belts: bigint[] = [];
...
function normalizePackedBlobBelts(belts: readonly bigint[]): bigint[] | null {
  if (belts.length === 0) return null;
  const byteLen = Number(belts[0]);
  if (!Number.isFinite(byteLen) || byteLen < 0 || byteLen > MAX_BLOB_UTF8_BYTES) return null;
...
export function decodeLenPrefixedBlob(belts: readonly bigint[]): Uint8Array | null {
...
function packedBlobNoun(bytes: Uint8Array): Noun {
  return toWire(vecToNoun(encodeBlobBelts(bytes)));
}

function pushEntry(data: NoteData, key: string, noun: Noun): NoteData {
  const filtered = data.filter(([k]) => k !== key);
  return [...filtered, [key, noun]];
}

export function noteDataPushMemo(data: NoteData, memoUtf8: string): NoteData {
...
export function noteDataPushBlob(data: NoteData, blobUtf8: string): NoteData {
...
export function decodePackedBlobUtf8(jamBytes: Uint8Array): string | null {
...
export function decodeNoteDataPackedUtf8(noun: Noun): string | null {
